...
def obtener_precio(row, cantidad):
    ...
    for i, thresh in enumerate(thresholds):
        if cantidad < thresh:
            # usar el anterior
            if i == 0:
                return float(row['precio_1'])
            return float(row[cols[i-1]])
    # si es mayor a 3000
...
<head><meta charset="utf-8"><style>
    ...
    body { font-family: Arial, sans-serif; margin: 0; padding: 40px; line-height: 1.4; }
    .header { text-align: right; font-size: 11px; }
    .titulo { text-align: center; font-size: 22px; font-weight: bold; margin: 30px 0; }
...
<h1 style="text-align:center; margin-top:80px;">Cotización</h1>
...
Estos términos se presentan con la finalidad de entregarte la información de la forma más clara posible y que el proceso sea completamente transparente en ambas direcciones, buscando que esta interacción sea constante y de enriquecimiento mutuo.<br><br>
Gracias por confiar en nosotros para acompañarte.
</p>
<div class="footer">www.terret.co • CI. 46A Sur #4881, Zona 1, Envigado, Antioquia, Colombia • @terretsports • info@terret.co</div>

<!-- PÁGINA 2 - PORTAFOLIO -->
<div class="page-break">
    <h2 class="titulo">PORTAFOLIO CAMISETAS</h2>
    <p style="text-align:center;">LINEA BOSTON • MINIMO 5 UND • LINEA CHICAGO • MINIMO 5 UND</p>
    <!-- Aquí puedes agregar imágenes y tabla de especificaciones de tela más adelante -->
</div>
...
<!-- PÁGINA 4 - MERCH POR CONVENIO -->
<div class="page-break">
    <h2 class="titulo">PROPUESTA COMERCIAL MERCH, POR CONVENIO ANUAL</h2>
    <!-- Puedes dejar la tabla fija o hacerla dinámica más adelante -->
    <p style="text-align:center;">(Tabla de convenio anual - puedes copiarla manualmente por ahora)</p>
</div>
...
<div class="page-break">
    ...
    <ul>
        ...
        <li>Visualización en las redes sociales de TERRET 40.200 seguidores...</li>
        <!-- copia el resto de tu página 5 aquí -->
    </ul>
</div>
...
<div class="page-break">
    ...
    <p style="text-align:justify; font-size:11px;">
        Opciones de Diseño • Insumos de tu parte • Tiempo de Producción • Ficha Técnica • Pagos 50% anticipo • etc.
    </p>
    <p style="margin-top:100px; text-align:center;">FIRMA CLIENTE ___________________<br>JUAN FELIPE GÓMEZ - DIRECTOR DE MARCA - TERRET SAS</p>
    <div class="footer">www.terret.co • CI. 46A Sur #4881, Zona 1, Envigado, Antioquia, Colombia • @terretsports • info@terret.co</div>
</div>
...
titulo_propuesta = st.selectbox("Título de la propuesta", titulos + ["Personalizado..."])
if titulo_propuesta == "Personalizado...":
    titulo_propuesta = st.text_input("Escribe título personalizado")
...
with col1:
    producto_sel = st.selectbox("Producto", productos_df["nombre_producto"])
with col2:
    cantidad = st.number_input("Cantidad", min_value=1, value=1)
with col3:
    if st.button("➕ Agregar"):
        row = productos_df[productos_df["nombre_producto"] == producto_sel].iloc[0]
        precio_unit = obtener_precio(row, cantidad)
...
# ====================== GENERAR PDF ======================
if st.button("🚀 Generar PDF completo de 6 páginas", type="primary") and st.session_state.items:
    numero = obtener_siguiente_numero()

    # Crear tablas HTML para página 3
    tablas_html = ""
    ...
    st.success(f"✅ Cotización #{numero:04d} generada y guardada en Drive")
    st.balloons()
